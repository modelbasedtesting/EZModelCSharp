...
int HypView::initGraph(string & rootId, int rootPriority, string & rootGroup)
{
  if (hypGraph) {
    delete(hypGraph);
    hypGraph = (HypGraph *)0;
...
int HypView::setColorLink(const string & fromid, const string & toid, float r, float g, float b)
{
  if (!hypGraph)
    return 0;
  HypNode *fromnode = hypGraph->getNodeFromId(fromid);
...
int HypView::setDrawLink(const string & fromid, const string & toid, bool on)
{
  if (!hypGraph)
    return 0;
  HypNode *fromnode = hypGraph->getNodeFromId(fromid);
...
void HypView::setColorGroup(int i, const string & group, float r, float g, float b)
{
  if (!hypGraph) return;
...
void HypView::setNodeGroup(const string & id, int groupkey, const string & group)
{
  if (!hypGraph) return;
